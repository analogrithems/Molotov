...
$config['namespaces']['Auth\Models'] = MODULES_DIR."/Auth/Models";
$config['namespaces']['Auth\Controllers'] = MODULES_DIR."/Auth/Controllers";
...
$di->setShared('session', function() use($di){
	return  \Auth\Controllers\Session::instance($di);
});
...
$app->map('/Auth',function() use ($app,$di){
	...
	$assets->addJs('Modules/Auth/Views/Web/Auth/js/login.js');
	$assets->addJs('Modules/Auth/Views/Web/Auth/js/passwordReset.js');
	$assets->addJs('Modules/Auth/Views/Web/Auth/js/passwordResetRequest.js');
	$assets->addJs('Modules/Auth/Views/Web/Auth/js/dashboard.js');
	$assets->addJs('Modules/Auth/Views/Web/Auth/js/group.js');
...
$app->map('/Signup',function() use ($app,$di){
	...
	$assets->addJs('js/modules/model.js');
	$assets->addJs('js/modules/router.js');
	$assets->addJs('Modules/Auth/Views/Web/Auth/js/signup.js');
	$view->setViewsDir($viewPath);
	echo $view->render('index',array('assets'=>$assets));
...
$app->map('/api/Auth/AddUser',function() use ($app){
	...
	$_user = new \Auth\Controllers\User();
...
$app->map('/api/Auth/SignupActivation',function() use ($app){
	...
	$args['email'] = $app->request->get('email');
	$args['activation_key']= $app->request->get('activation_key');
	$user = new \Auth\Controllers\User();
...
$app->map('/api/Auth/Group',function() use ($app,$di){
	...
	$group = new \Auth\Controllers\Group();
...
$app->map('/api/Auth/Group/{id}/Members',function($id) use ($app){
	$members = array();
	$groupController = new \Auth\Controllers\Group();
...
$app->map('/api/Auth/Role',function() use ($app,$di){
	...
	if( $di->get('session')->can('manage_group',$group_id) ){
		$roleContoller = new \Auth\Controllers\Role();
...
$app->map('/api/Auth/Capabilities',function() use ($app){
	$capabilityController = new \Auth\Controllers\Capability();
