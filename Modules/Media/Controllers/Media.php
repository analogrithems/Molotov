...
class Media extends BaseController{
	...
	public function action_getFolderInfo( $id ){
		$folder = \Media\Models\Folders::findFirst($id);
		$result = array( 
			'id'=>$folder->id,
			'name'=>$folder->name,
		...
		if( $folder->parent > 0 ){
			$step = $folder->parent;
			while($step > 0){
				$_tmp = \Media\Models\Folders::findFirst($step);
		...
		$mediaResults = \Media\Models\Media::find(
			array(
				"folder_id = :folder_id: ",
				"bind" => array(
					"folder_id"=>$id
		...
		foreach($mediaResults as $media){
			...
			$metas = \Media\Models\Meta::find(array("media_id = :id:","bind"=>array("id"=>$media->id)));
		...
		$childFolders = \Media\Models\Folders::find(
			array(
				'parent = :parent_id:',
				'bind'=>array(
					'parent_id'=>$id
	...
	public function action_rootFolderInfo( $repo ){
		...
		$folders = \Media\Models\Folders::find( $filter );
		$result = array( 
			'total'=>\Media\Models\Folders::count( $filter ),
			'repo'=>$repo,
			'status'=>'ok'
		);
	...
	public function action_getMediaInfo( $id ){
		$media = \Media\Models\Media::findFirst($id);
	...
	public function getMediaInfo( $id ){
		$id = (int)$id;
		$folder = \Media\Models\Media::findFirst($id);
		return $folder->serialize();
	}
